...
const RuntimeModeControls: React.FC<{ showDebugPanel: boolean }> = ({ showDebugPanel }) => {
  ...
  const capabilityIndicators = useMemo(
    () => [
      { label: 'Map', value: capabilities.hasMap, icon: 'Map' },
      { label: 'Camera', value: capabilities.hasCamera, icon: 'Cam' },
      { label: 'Minimap', value: capabilities.hasMinimap, icon: 'Min' },
      { label: 'Navigation', value: capabilities.supportsNavigation, icon: 'Nav' }
    ],
    [capabilities]
  );
      ...
      <div className="grid grid-cols-4 gap-2 pt-2 border-t border-gray-700">
        {capabilityIndicators.map((cap) => (
          ...
          >
            ...
            <div className="text-[9px] opacity-60">{cap.value ? 'Y' : 'N'}</div>
          </div>
        ))}
      </div>
