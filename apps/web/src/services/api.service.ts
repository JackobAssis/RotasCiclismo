...
export class ApiService {
  ...
  async updateProfile(
    ...
    return apiClient.patch<UserProfileDto>(
      API.USER_ENDPOINTS.UPDATE_PROFILE(id),
      dto
    );
  }
  ...
  async listRides(
    ...
    return apiClient.get<PaginatedResponseDto<RideDto>>(
      `${API.RIDES_ENDPOINTS.LIST}?${params}`
    );
  }
  ...
  async getRideWithRoute(id: string): Promise<RideDetailDto> {
    return apiClient.get<RideDetailDto>(
      API.RIDES_ENDPOINTS.GET_WITH_ROUTE(id)
    );
  }

  async updateRide(id: string, dto: UpdateRideDto): Promise<RideDto> {
    return apiClient.patch<RideDto>(
      API.RIDES_ENDPOINTS.UPDATE(id),
      dto
    );
  }

  async finishRide(id: string, dto: FinishRideDto): Promise<RideDto> {
    return apiClient.post<RideDto>(
      API.RIDES_ENDPOINTS.FINISH(id),
      dto
    );
  }
  ...
  async createRoutePoint(
    ...
    return apiClient.post<RoutePointDto>(
      API.ROUTE_POINTS_ENDPOINTS.CREATE(rideId),
      dto
    );
  }
  ...
  async createBulkRoutePoints(
    ...
    return apiClient.post<{ created: number }>(
      API.ROUTE_POINTS_ENDPOINTS.CREATE_BULK(rideId),
      dto
    );
  }
  ...
  async deleteRoutePoints(
    rideId: string
  ): Promise<{ deleted: number }> {
    return apiClient.delete<{ deleted: number }>(
      API.ROUTE_POINTS_ENDPOINTS.DELETE(rideId)
    );
  }
  ...
  async createSnapshot(
    ...
    return apiClient.post<SnapshotDto>(
      API.SNAPSHOTS_ENDPOINTS.CREATE(rideId),
      dto
    );
  }
  ...
  async updateSnapshotStatus(
    id: string,
    dto: UpdateSnapshotStatusDto
  ): Promise<SnapshotDto> {
    return apiClient.patch<SnapshotDto>(
      API.SNAPSHOTS_ENDPOINTS.UPDATE_STATUS(id),
      dto
    );
  }
  ...
  async deleteSnapshot(id: string): Promise<{ success: boolean }> {
    return apiClient.delete<{ success: boolean }>(
      API.SNAPSHOTS_ENDPOINTS.DELETE(id)
    );
  }
  ...
  async getPendingSyncTasks(
    ...
    return apiClient.get<SyncTaskDto[]>(
      `${API.SYNC_ENDPOINTS.GET_PENDING}?${params}`
    );
  }
  ...
  async markSyncTaskCompleted(id: string): Promise<{ success: boolean }> {
    return apiClient.post<{ success: boolean }>(
      API.SYNC_ENDPOINTS.MARK_COMPLETED(id)
    );
  }

  async markSyncTaskFailed(
    id: string,
    error: string
  ): Promise<{ success: boolean }> {
    return apiClient.post<{ success: boolean }>(
      API.SYNC_ENDPOINTS.MARK_FAILED(id),
      { error }
    );
  }
